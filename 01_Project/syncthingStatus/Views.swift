        ...
        Form {
            ...
            Section("Notifications") {
                ...
                if settings.showSyncNotifications {
                    VStack(alignment: .leading, spacing: AppConstants.UI.spacingM) {
                        HStack {
                            Text("Min. interval per folder")
                            Spacer()
                            Text(notificationCooldownMinutes <= 0
                                 ? "Off"
                                 : "\(Int(notificationCooldownMinutes)) min")
                                .foregroundColor(.secondary)
                        }
                        Slider(value: $notificationCooldownMinutes, in: 0...60, step: 1)
                            .onChange(of: notificationCooldownMinutes) { _, newValue in
                                settings.syncNotificationCooldownMinutes = newValue
                            }
                        Text("Skip a notification if one for the same folder fired within this many minutes. Set to Off (0) to notify on every completion.")
                            .font(.caption2)
                            .foregroundColor(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
