...
final class SyncthingStatusIcon: NSObject {
    ...
    private let staticIcons: [SyncState: String] = [
        ...
        .warning: "syncthingStatus-WARN",
        .error: "syncthingStatus-ERROR"
    ]

    /// Asset name to fall back to when the primary asset for a state isn't
    /// bundled. Lets `.warning` ship with code before the WARN PNG exists —
    /// the icon will silently render as Normal until the asset is added.
    private let staticIconFallbacks: [SyncState: String] = [
        .warning: "syncthingStatus-Normal"
    ]
    ...
    func set(state: SyncState) {
        ...
        case .normal:
            ...
            currentFrameNames = []
            currentTooltip = "In sync"
            applyStaticIcon(for: .normal, tooltip: "In sync")

        case .warning:
            stopAnimation()
            currentFrameNames = []
            currentTooltip = "Warning"
            applyStaticIcon(for: .warning, tooltip: "Warning")

        case .error:
            stopAnimation()
            currentFrameNames = []
            currentTooltip = "Error"
            applyStaticIcon(for: .error, tooltip: "Error")

        // The three sync states render the same static SYNCING icon for now.
        // Animated frame sets (frameSets / startAnimation) are intentionally
        // retained in this file so the animated behavior can be restored
        // later by replacing the body of these three cases with the original
        // startAnimation calls — no other code needs to change.
        case .uploading:
            stopAnimation()
            currentFrameNames = []
            currentTooltip = "Uploading…"
            applySyncingIcon(tooltip: "Uploading…")

        case .downloading:
            stopAnimation()
            currentFrameNames = []
            currentTooltip = "Downloading…"
            applySyncingIcon(tooltip: "Downloading…")

        case .upAndDown:
            stopAnimation()
            currentFrameNames = []
            currentTooltip = "Syncing…"
            applySyncingIcon(tooltip: "Syncing…")
        }
    }
    ...
    private func applyStaticIcon(for state: SyncState, tooltip: String) {
        if let primary = staticIcons[state], image(named: primary) != nil {
            applyImage(named: primary, tooltip: tooltip)
            return
        }
        if let fallback = staticIconFallbacks[state], image(named: fallback) != nil {
            applyImage(named: fallback, tooltip: tooltip)
        }
    }

    private func applySyncingIcon(tooltip: String) {
        if image(named: "syncthingStatus-SYNCING") != nil {
            applyImage(named: "syncthingStatus-SYNCING", tooltip: tooltip)
            return
        }
        if let fallback = staticIcons[.normal] {
            applyImage(named: fallback, tooltip: tooltip)
        }
    }
