...
| A6 | Legacy notification settings load; user switches off the final selected folder | Legacy empty migrates to explicit All; explicit empty selection means no per-folder completion notices, survives relaunch, and matches delivered folder notifications. The separately named global “All Synced” notice remains controlled by the master toggle. |
| A7 | Pause/resume overlaps unrelated daemon edits or a newer same-folder intent | Only the target folder's paused field changes; an in-flight mutation resolves before the latest intent is reconciled; failure stays visible and does not emit success. |
| A8 | Cleanup candidates span pages, a page is malformed, or a later page fails/is cancelled | Candidates beyond 1,000 are reachable and deduplicated; only a strictly decoded, terminal generation is actionable, and incomplete/stale results cannot become a complete list. |
| A9 | First launch, missing/revoked config access, invalid key or connection failure | Connection settings appear first; a typed recovery action, full error and direct picker/manual recovery are available. Notification permission waits for connection or explicit intent and is not re-requested after denial. |
...
- Waves **1–3** are complete and A1–A5 passed. Wave 4 was selected and implemented serially because `Client.swift`, `Views.swift` and `App.swift` overlap; its isolated live acceptance remains required. Tasks in one wave are not automatically safe for parallel edits. Delegate bounded fixture work/review to Sol or Luna with explicit ownership when the host permits it. Split a numbered task further if it cannot fit a short implementation pass.
...
- [ ] **4.1 — Persist explicit notification selection.** Targets: `SyncthingSettings.swift` keys/init/save/reset, client notification predicate, existing per-folder Settings section, `NotificationSelectionTests.swift`.
  - Success: place All/Selected mode beside the existing folder list. Legacy empty/missing migrates to All; nonempty to Selected. Selected empty means no per-folder completion notifications. In All, every row shows enabled and new folders are included; turning one off creates an explicit remaining selection. Persist mode and deduplicated IDs. The mode scopes per-folder completion notices only: keep the existing global “All Synced” notice under the master sync-notification toggle and label the folder mode so None cannot be mistaken for disabling that global notice. Retain cooldown behavior (A6).
  - Backpressure: T(NotificationSelectionTests) for migration (including current defaults and old-bundle import), reset, reload, deduplication, all/one/none and captured per-folder delivery; assert the documented global-notice behavior separately. V for switches, labels and relaunch.
  - Implemented 2026-09-11 in `5633ec3`; automated cases pass. Awaiting the isolated preference/UI/relaunch portion of V.

- [x] **4.2 — Patch only the requested pause state.** Targets: `Client.swift` (`setFolderPausedState`, narrow JSON helper), `FolderPauseTests.swift`.
  - Success: PATCH individual folder endpoint with only `paused`, correct ID encoding and JSON headers; no whole-config write/fallback. Different-folder requests preserve unrelated edits; opposite actions on one folder serialize/coalesce to latest intent. Do not cancel an in-flight mutation when a newer intent arrives because cancellation cannot prove the daemon did not apply it; let it resolve, then apply the latest queued intent and reconcile through the refresh owner. Notify/update only after confirmed success. Handle 401/403/404/server failure visibly (A7).
  - Backpressure: T(FolderPauseTests) asserting method/path/body, concurrent different-folder state, rapid same-folder opposite intents, an in-flight request that applies before client cancellation, final reconciled state and no success notification on failure; B. Use a disposable daemon for any live mutation test.
  - Completed 2026-09-11 in `5633ec3` + `f7bc4dc`: request/order/connection-transition regressions, B, independent review, and a production-source harness against a hardened disposable Syncthing v2.1.2 daemon passed. Only targeted pause/resume PATCHes occurred and the unrelated sentinel stayed paused. [Evidence](reviews/evidence/2026-09-11/wave-4.md).
...
- [ ] **4.3 — Load complete cleanup pages with cancellation.** Targets: client `fetchDbNeed`, controller `loadCandidates`, `Models.swift` response, cleanup loading UI, `CleanupPaginationTests.swift`.
  - Success: paginate on demand across progress/queued/rest until a short/empty raw page; termination uses raw count before filtering/deduplication, never `total`. For this destructive workflow, missing or wrongly typed response buckets fail closed instead of decoding as empty; validate returned page/per-page metadata and reject repeated, mismatched or nonprogressing pages. Accumulate privately, deduplicate relative identities; conflicting duplicates fail the load for retry rather than guess deletion eligibility. Publish candidates atomically only after the terminal page and record that generation as complete. Deletion requires that completed generation; a failed, cancelled or superseded reload cannot leave an older list actionable. Close/reload/identity change cancels obsolete publication and pagination stops between pages (A8).
  - Backpressure: T(CleanupPaginationTests) for 0/1,000/1,001, exact multiples, mixed arrays, duplicates/conflicts, full pages with no candidates, missing/wrong-type buckets, missing/mismatched page metadata, repeated page, page-two failure and cancellation, and stale-list non-actionability; T(CleanupSafetyTests), V.
  - Implemented 2026-09-11 in `5633ec3` + `f7bc4dc`; strict pagination and the full cleanup suite pass. A production-controller harness passed 1,001-candidate, later-page failure, malformed-page, and cancellation scenarios against a controlled responder. Awaiting the cleanup-window UI portion of V.
...
- [ ] **4.4 — Reuse the picker for direct connection recovery.** Targets: `Views.swift` Settings/Disconnected/Footer and `selectSyncthingConfig`, small shared picker action if needed, client credential errors, `App.swift` permission trigger, `ConnectionRecoveryTests.swift`.
  - Success: automatic/manual Connection sections lead Settings. Publish a typed recovery kind/action (config access, credentials, URL or transport) beside the human-readable error; UI action selection must not match localized message strings. Show the full error beside disconnected recovery controls; reuse the picker for missing/revoked config access and manual settings for key/URL errors. Preserve the real-home picker hint; discovery does not grant sandbox access. Extract the notification-authorization decision into a small production policy/coordinator included in the hostless test target. Request permission only when status is not determined and either the first successful connection has notifications enabled or the user expresses explicit notification intent; prior denial does not trigger another request and instead remains a settings-recovery case (A9).
  - Backpressure: T(ConnectionRecoveryTests) for typed error/action mapping and injected authorization states (`notDetermined`, allowed and denied), first connection, explicit intent, repeated events and target membership. V using isolated clean preferences, picker cancel/regrant, bad config/key, loopback HTTPS and denied permission.
  - Implemented 2026-09-11 in `5633ec3` + `f7bc4dc`; typed-policy/concurrency tests, visible denied-state recovery, B, independent review, and read-only Connection-first AX inspection pass. Awaiting isolated picker/error/TCC UI cases.
...
- [ ] **4.5 — Verify combined recovery/action flows.** Depends on 4.1–4.4. Targets: integration evidence and task/state docs.
  ...
  - Automated integration, B, D and independent review passed 2026-09-11. The combined isolated V matrix remains, so this task is not complete.
...
Wave 4 implementation checkpointed on 2026-09-11. Tasks 4.1–4.4 were executed serially under one owner because their production files overlap, then integrated through 4.5. The full 104-test suite, ad-hoc Debug build, `git diff --check`, local diff review and read-only Connection-first Settings AX inspection passed; the exact fresh artifact was launched. Agent delegation/independent review was unavailable under the active host instructions. The required isolated preference, picker/TCC/error, controlled pagination and disposable-daemon mutation scenarios were not run against the user's live state, so 4.1–4.5 and A6–A9 remain unchecked. [Evidence](reviews/evidence/2026-09-11/wave-4.md). The validated source is committed locally as `5633ec3`; no push or release is authorized.
